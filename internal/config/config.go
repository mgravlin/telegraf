...
func buildParser(name string, tbl *ast.Table) (parsers.Parser, error) {
	...
	if node, ok := tbl.Fields["seperator"]; ok {
		if kv, ok := node.(*ast.KeyValue); ok {
			if str, ok := kv.Value.(*ast.String); ok {
...
func buildSerializer(name string, tbl *ast.Table) (serializers.Serializer, error) {
	...
	if node, ok := tbl.Fields["source_override"]; ok {
		if kv, ok := node.(*ast.KeyValue); ok {
			if ary, ok := kv.Value.(*ast.Array); ok {
				for _, elem := range ary.Value {
					if str, ok := elem.(*ast.String); ok {
						c.SourceOverride = append(c.SourceOverride, str.Value)
					}
				}
			}
		}
	...
	if node, ok := tbl.Fields["host_tag"]; ok {
		if kv, ok := node.(*ast.KeyValue); ok {
			if str, ok := kv.Value.(*ast.String); ok {
				c.HostTag = str.Value
			}
	...
	delete(tbl.Fields, "source_override")
	delete(tbl.Fields, "host_tag")
